...
void run_stats::print_quantile_latency_column(output_table &table, double quantile, char* label) {
    ...
    if (print_arbitrary_commands_results()) {
        ...
        const bool has_wait_ops = hdr_total_count(m_wait_latency_histogram) > 0;
        if(has_set_ops){
            if(abs(quantile - 99) < 1e-3) {
                ofstream percentile_file("logs/dump/latency_percentile.txt");
                percentile_file << hdr_value_at_percentile(m_set_latency_histogram, quantile )/ (double) LATENCY_HDR_RESULTS_MULTIPLIER << endl;
        ...
        if(has_get_ops){
            if(abs(quantile - 99) < 1e-3) {
                ofstream percentile_file("logs/dump/latency_percentile.txt");
                percentile_file << hdr_value_at_percentile(m_get_latency_histogram, quantile )/ (double) LATENCY_HDR_RESULTS_MULTIPLIER << endl;
                percentile_file.close();
            }
